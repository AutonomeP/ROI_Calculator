        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {systems.map((s, i) => (
              <tr key={i} className={`border-t transition-colors ${isDark ? 'border-white/5 hover:bg-white/[0.04]' : 'border-black/5 hover:bg-black/[0.025]'}`}>
                ...
                <td className="px-4 py-3 text-right text-autonome-blue font-semibold">{formatCurrency(s.roiResult.monthly_leveraged_value)}</td>
                ...
                <td className="px-4 py-3 text-right">
                  <span className={`px-2 py-0.5 rounded-lg text-xs font-semibold ${
                    s.inputs.solutionMode === 'agentic_intelligent_ai'
                      ? 'bg-autonome-yellow/20 text-black'
                      : 'bg-autonome-blue/10 text-autonome-blue'
                  }`}>
          ...
          <tfoot>
            <tr className={`border-t font-semibold ${isDark ? 'border-white/20 bg-white/[0.045]' : 'border-black/15 bg-black/[0.035]'}`}>
              ...
              <td className="px-4 py-3 text-right text-autonome-blue">{formatCurrency(systems.reduce((s, r) => s + r.roiResult.monthly_leveraged_value, 0))}</td>
